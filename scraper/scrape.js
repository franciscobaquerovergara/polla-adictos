...
async function main() {
  ...
  const EMAIL = process.env.POLLAYA_EMAIL || '';
  const PASS = process.env.POLLAYA_PASSWORD || '';
  ...
  try {
    ...
    const contTexts = ['Continuar', 'Continúa', 'Continua', 'Siguiente', 'Ingresar', 'Entrar', 'Acceder', 'Continue', 'Next'];

    // Pollaya's login is reached from the home page and is two-step (email, then
    // password). Try /login and /, click a sign-in entry point, and if an email
    // field shows up without a password, submit it to reveal the password step.
    let passField = null;
    for (const url of [BASE_URL + '/login', BASE_URL + '/']) {
      ...
      for (const txt of loginTexts) {
        ...
        passField = await findPass(1200);
        if (passField) break;
      }
      if (passField) break;

      // Two-step: email field present but no password yet.
      const ef = await page.$(emailSelector).catch(() => null);
      if (ef) {
        await ef.fill(EMAIL).catch(() => {});
        let cont = false;
        for (const txt of contTexts) {
          const b = await page.$('button:has-text("' + txt + '")').catch(() => null);
          if (b) { await b.click().catch(() => {}); cont = true; break; }
        }
        if (!cont) await ef.press('Enter').catch(() => {});
        passField = await findPass(15000);
        if (passField) break;
      }
    }
    if (!passField) passField = await findPass(15000);
    ...
    if (!passField) {
      try {
        const diag = await page.evaluate(() => ({
          ...
          buttons: [...document.querySelectorAll('button')].map(b => (b.textContent || '').trim().slice(0, 25)).filter(Boolean).slice(0, 15),
    ...
    if (emailField) await emailField.fill(EMAIL).catch(() => {});
    await passField.fill(PASS);
    await passField.press('Enter');

    await page.waitForFunction(() => !location.href.includes('/login') && !!document.querySelector('.t-pts, .name, [href*="mis-grupos"]'), { timeout: 20000 }).catch(() => {});
